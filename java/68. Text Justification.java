...
/**
Complexity:
Time Complexity: Both approaches process each word once and have a time complexity of O(n)O(n)O(n), where nnn is the number of words.

Space Complexity: The space complexity for both methods is O(n×m)O(n \times m)O(n×m), where nnn is the number of words and mmm is the average length of the words.
 */
